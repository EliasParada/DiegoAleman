async function cargarProyectos(language) {
    try {
      ...
      sectionProyectos.innerHTML = `
        ...
        <h2 class="text-xl md:text-6xl font-bold mb-8 text-white" data-es="${data.es.subtitle}" data-en="${data.en.subtitle}">
          ${content.subtitle}
        </h2>
      `;
      ...
      content.projects.forEach((project, index) => {
        ...
        card.innerHTML = `
          ...
          <div class="mt-4">
            ...
            <button onclick="openModal(${index})" class="mt-4 bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded flex items-center gap-2">
  ...
  function openModal(index) {
    ...
    modal.innerHTML = `
      <button onclick="closeModal()" class="absolute top-4 right-4 bg-red-600 text-white rounded-full p-2 z-10">
        <i class="fa-solid fa-times"></i>
      </button>
      <div class="swiper-container w-full h-64">
        <div class="swiper-wrapper">
          ${project.carousel
            .map(image => `<div class="swiper-slide"><img src="${image}" alt="" class="w-full h-full object-cover"></div>`)
            .join('')}
        </div>
      ...
      <div class="mt-6">
        ...
        <div class="mt-6 grid grid-cols-2 gap-4">
          ${project.gallery
            .map(image => `<a href="${image}" data-lightbox="gallery" class="overflow-hidden rounded-md"><img src="${image}" alt="" class="w-full h-32 object-cover"></a>`)
            .join('')}
        </div>
      </div>
    `;
